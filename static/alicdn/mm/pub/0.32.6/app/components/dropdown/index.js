/**
 * dropdown组件修改版
 * 1、移除了keydown事件中default分支
 * 2、修改了只有input[type=hidden]才会赋值
 * 注意：
 * 目前只有带有搜索功能的dropdown才引用这个版本，
 * 其他情况仍然引用线上版本
 */

KISSY.add("components/dropdown/index", function(S, Brick) {
    /**
     * 下拉选择
            ...
            keydown:function(e){
                ...
                switch (e.keyCode) {
                    ...
                    case 38:
                        ...
                        hoverItem = item.prev(function(node){
                    ...
                    case 40:
                        ...
                        hoverItem = item.next(function(node){
                            if(node.style.display != 'none') {
                                return true;
                            }
                        });
    ...
    S.extend(Dropdown, Brick, {
        ...
        _select:function(selectedItem,flg){
            ...
            var inputNode = el.one('input[type=hidden]');
            if(inputNode){
        ...
        _hover:function(item,isScroll){
            ...
            if(isScroll){
                item.scrollIntoView(item.parent('ul'));
            }
        }
    });
