Magix.tmpl("app/views/report/zone/zone_extra","<div class=\"wrap-hd clearfix\"> <div class=title-bar> <h2 class=title>内容合作</h2> </div> </div> <div class=table-container> <div class=\"toolbar z-index-2 clearfix\"> <div class=table-settings bx-name=\"chart_toggle\" bx-path=\"components/chart_toggle/\"> <a href=\"#\" class=\"btn btn-size25 btn-active chart-toggle\"><i class=iconfont>&#254;</i></a> <span bx-tmpl=\"download\" bx-datakey=\"productType,startTime,endTime\"> <a href=\"/report/thirdRpt.json?DownloadID=DOWNLOAD_REPORT_THIRD&productType={{productType}}&startTime={{startTime}}&endTime={{endTime}}\" class=\"btn btn-size25\" title=\"下载报表\"><i class=iconfont>&#392;</i></a> </span> </div> <div bx-name=\"dropdown\" class=dropdown hidefocus=true id=J_type_dropdown style=\"width:100px;\"> {{#typeList}} {{#selected}} <span class=dropdown-hd> <span class=dropdown-text value=\"{{id}}\">{{name}}</span> <i class=\"iconfont icon-arrow-down\">&#459</i> </span> {{/selected}} {{/typeList}} <ul class=dropdown-list style=\"max-height:300px;\"> {{#typeList}} <li class=\"dropdown-item {{#selected}}dropdown-itemselected{{/selected}}\" title=\"{{name}}\"><span value=\"{{id}}\">{{name}}</span><i class=\"iconfont icon-ok\">&#126</i></li> {{/typeList}} </ul> </div> <a href=\"#\" class=\"btn btn-size25\" id=sitemapTimeRange bx-tmpl=\"dateRange\" bx-datakey=\"startTime,endTime\"> {{startTime}} 至 {{endTime}} </a> </div> <table class=\"table report-count\" bx-tmpl=\"countData\" bx-datakey=\"countData\"> <tbody> <tr> {{#countData}} <td class=\"center {{#__last__}}last{{/__last__}}\" width=\"25%\"> <dl> <dt> {{label}} <i class=\"iconfont cursor-pointer fontsize-13 ml2\" bx-name=\"tooltip\" bx-config=\"{closable:false,align:{node:false,points:{{#__last__}}['tr','br']{{/__last__}}{{^__last__}}['tl','bl']{{/__last__}}},mouseDelay:0.2,width:300,content:'{{tip}}'}\">&#360;</i> </dt> <dd><strong class=fontsize-18>{{value}}</strong></dd> </dl> </td> {{/countData}} </tr> </tbody> </table> <div class=chart-container> <vframe id=magix_vf_chartView> <div class=wrap-loading></div> </vframe> </div> <vframe mx-view=\"app/views/report/zone/zone_extra_table\"> <div class=wrap-loading></div> </vframe> </div>");
KISSY.add('app/views/report/zone/zone_extra', function (S, View, VOM, Node, MM, Util) {
    var $ = Node.all;
    return View.extend({
        init: function (e) {
            var me = this;
        ...
        components: function () {
            ...
            typeDropdown.on('selected', function (ev) {
                me.navigate('pageNo=1&typeId=' + ev.value);
            });
            me._initChart();
        },
        _initChart: function () {
